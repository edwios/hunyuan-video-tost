...
RUN pip install torch==2.5.1+cu124 torchvision==0.20.1+cu124 torchaudio==2.5.1+cu124 torchtext==0.18.0 torchdata==0.8.0 --extra-index-url https://download.pytorch.org/whl/cu124 && \
    ...
    aria2c --console-log-level=error -c -x 16 -s 16 -k 1M https://huggingface.co/carmenduru/HunyuanVideo/resolve/main/hunyuan-video-t2v-720p/transformers/mp_rank_00_model_states.pt -d /workspace/HunyuanVideo/ckpts/hunyuan-video-t2v-720p/transformers -o mp_rank_00_model_states.pt && \
    aria2c --console-log-level=error -c -x 16 -s 16 -k 1M https://huggingface.co/carmenduru/HunyuanVideo/raw/main/hunyuan-video-t2v-720p/vae/config.json -d /workspace/HunyuanVideo/ckpts/hunyuan-video-t2v-720p/vae -o config.json && \
    aria2c --console-log-level=error -c -x 16 -s 16 -k 1M https://huggingface.co/carmenduru/HunyuanVideo/resolve/main/hunyuan-video-t2v-720p/vae/pytorch_model.pt -d /workspace/HunyuanVideo/ckpts/hunyuan-video-t2v-720p/vae -o pytorch_model.pt && \
